...
class AdvGAN(LightningModule):
    ...
    def forward(self, imgs):
        perturbations, res_imgs = self.generate_restored_imgs(imgs)
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        adv_imgs = self.attack(imgs)

        perturbation, res_imgs = self.generate_restored_imgs(adv_imgs)

        if optimizer_idx == 0:
            losses = self.generator_losses(imgs, labels, res_imgs, perturbation, 'train')

            return losses["train_loss_generator"]

        if optimizer_idx == 1:
            losses = self.discriminator_loss(imgs, res_imgs, 'train')

            return losses["train_loss_discriminator"]

        if optimizer_idx == 2:
            losses = self.distillation_loss(imgs, labels, res_imgs, 'train')
    ...
    def validation_step(self, batch, batch_idx):
        ...
        perturbation, res_imgs = self.generate_restored_imgs(adv_imgs)
        losses = self.generator_losses(imgs, labels, res_imgs, perturbation, 'validation')

        labels_original_pred, labels_restored_pred, labels_adversarial_pred = self.target_model_metrics(imgs, labels, res_imgs, adv_imgs)

        return imgs, labels, perturbation, res_imgs, labels_original_pred, labels_restored_pred, adv_imgs, labels_adversarial_pred

    def validation_epoch_end(self, outputs):
        imgs_batches, labels_batches, perturbation_batches, res_imgs_batches, labels_original_pred_batches, labels_restored_pred_batches, adv_imgs_batches, labels_adversarial_pred_batches = [
            torch.stack([output[i] for output in outputs]) for i in
            range(len(outputs[0]))]

        wrongly_predicted_mask = labels_adversarial_pred_batches != labels_batches
        imgs_batches, labels_batches, perturbation_batches, res_imgs_batches, labels_original_pred_batches, labels_restored_pred_batches, adv_imgs_batches, labels_adversarial_pred_batches = imgs_batches[wrongly_predicted_mask],  labels_batches[wrongly_predicted_mask],  perturbation_batches[wrongly_predicted_mask],  res_imgs_batches[wrongly_predicted_mask],  labels_original_pred_batches[wrongly_predicted_mask],  labels_restored_pred_batches[wrongly_predicted_mask], adv_imgs_batches[wrongly_predicted_mask], labels_adversarial_pred_batches[wrongly_predicted_mask]  
        imgs_batches, labels_batches, perturbation_batches, res_imgs_batches, labels_original_pred_batches, labels_restored_pred_batches, adv_imgs_batches, labels_adversarial_pred_batches = imgs_batches[:self.num_samples_to_log], labels_batches[:self.num_samples_to_log], perturbation_batches[:self.num_samples_to_log], res_imgs_batches[:self.num_samples_to_log], labels_original_pred_batches[:self.num_samples_to_log], labels_restored_pred_batches[:self.num_samples_to_log], adv_imgs_batches[:self.num_samples_to_log], labels_adversarial_pred_batches[:self.num_samples_to_log]
        ...
        wandb.log({
            "pred_imgs": [
                ...
                ) for img, label, pred in zip(imgs_batches, labels_batches, labels_original_pred_batches)
            ],
            "pred_res_imgs": [
                wandb.Image(
                    res_img,
                    caption=f'Pred: {pred}, Label: {label}'
                ) for res_img, label, pred in zip(res_imgs_batches, labels_batches, labels_restored_pred_batches)
            ],
            "perturbation": [
                wandb.Image(
                    perturbation,
                    caption=f'Label: {label}'
                ) for label, perturbation in zip(labels_batches, perturbation_batches) 
            ],
            "pred_adv_imgs": [
                wandb.Image(
                    adv_img,
                    caption=f'Pred: {pred}, Label: {label}'
                ) for adv_img, label, pred in zip(adv_imgs_batches, labels_batches, labels_adversarial_pred_batches)
            ]
        })
    ...
    def target_model_metrics(self, imgs, labels, res_imgs, adv_imgs, stage='validation'):
        labels_original_pred = self.target_model_predict(imgs).argmax(1)
        labels_restored_pred = self.target_model_predict(res_imgs).argmax(1)
        ...
        accuracy_restored = accuracy(labels_restored_pred, labels)
        ...
        if self.is_distilled:
            ...
            labels_restored_pred_student = self.student_model(res_imgs).argmax(1)
            labels_adversarial_pred_student = self.student_model(res_imgs).argmax(1)

            accuracy_original_student = accuracy(labels_original_pred_student, labels)
            accuracy_restored_student = accuracy(labels_restored_pred_student, labels)
        ...
        losses = {
            ...
            f"{stage}_accuracy_restored_student": accuracy_restored_student if self.is_distilled else 0,
        ...
        return labels_original_pred, labels_restored_pred, labels_adversarial_pred

    def generate_restored_imgs(self, imgs):
    ...
    def generator_losses(self, imgs, labels, res_imgs, perturbation, stage='train'):
        if self.is_relativistic:
            loss_generator = self.generator_loss_relativistic(imgs, res_imgs)
        else:
            loss_generator = .05 * self.generator_loss_fake(res_imgs) + .9 * F.mse_loss(res_imgs, imgs)

        loss_perturb = self.perturbation_loss(perturbation)
        loss_adv = self.reverse_adversarial_loss(res_imgs, labels)
    ...
    def reverse_adversarial_loss(self, adv_imgs, labels):
