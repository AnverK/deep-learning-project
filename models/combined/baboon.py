...
class Baboon(LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        imgs_batches, labels_batches, adv_imgs_batches, res_imgs_batches, y_original_pred_batches, y_adversarial_pred_batches, y_restored_pred_batches = [torch.stack([output[i] for output in outputs])[:self.num_batches_to_log, :self.num_samples_to_log] for i in range(len(outputs[0]))]
        ...
        wandb.log({
            "pred_imgs": [
                ...
                ) for imgs, labels, preds in zip(imgs_batches, labels_batches, y_original_pred_batches) for img, label, pred in zip(imgs, labels, preds)
            ] if self.current_epoch == 0 else None,
            "pred_adv_imgs": [
                wandb.Image(
                    adv_img,
                    caption=f'Pred: {pred}, Label: {label}'
                ) for adv_imgs, labels, preds in zip(adv_imgs_batches, labels_batches, y_adversarial_pred_batches) for adv_img, label, pred in zip(adv_imgs, labels, preds)
            ],
            "pred_res_imgs": [
                wandb.Image(
                    res_img,
                    caption=f'Pred: {pred}, Label: {label}'
                ) for res_imgs, labels, preds in zip(res_imgs_batches, labels_batches, y_restored_pred_batches) for res_img, label, pred in zip(res_imgs, labels, preds)
            ]
        })
